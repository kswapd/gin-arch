...
function sendPostRequest(url, params, callBack, dataType, async, type) {
    var index1
    if (dataType === undefined || dataType === '' || dataType === null) { dataType = 'json' }
    if (async === undefined || async === '' || async === null) { async = true }
    if (type === undefined || type === '' || type === null) { type = 'POST' }
    $.ajax({
        type: type,
        url: url,
        async: async,
...
function dataTableUi(dataTableId, buttonName, buttonId) {
    if (buttonName === undefined) { buttonName = ['添加', '修改', '删除'] }
    if (buttonId === undefined) { buttonId = ['add', 'edit', 'delete'] }
...
/*  带按钮有分页的dataTable美化
 *  重庆需求 ：所有的变更模块中版本上线页面，按钮的排列：停止、增量部署、启动、刷新  （隔一段距离放置添加、修改、删除等）
 *  针对于重庆客户化需求修改
 */
function dataTableUiCq(dataTableId, buttonName, buttonId) {
    if (buttonName === undefined) { buttonName = ['添加', '修改', '删除'] }
    if (buttonId === undefined) { buttonId = ['add', 'edit', 'delete'] }
    $('#' + dataTableId + '_length').wrap('<div  class="cl table-header" id="' + dataTableId + '-header"></div>')
    $('#' + dataTableId + '_filter').appendTo('#' + dataTableId + '-header')
    for (var i in buttonName) {
        if (buttonName.hasOwnProperty(i)) {
            $('#' + dataTableId + '_length').before('<a  class="table-btn" id="' + buttonId[i] + '">' + buttonName[i] + '</a>')
            if (buttonName[i] == '刷新') {
            	 $('#' + dataTableId + '_length').before('<a>' + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' + '</a>')
            }
        }
    }
    $('#' + dataTableId + '_length').before($('#' + dataTableId + '_filter'))
        ...
        success: function (json) {
            if (json) {
                for (var i in json) {
                    if (json.hasOwnProperty(i)) {
                        var pageUrl = window.location.href
                        if (pageUrl.indexOf(json[i].id.split(',')[0]) != -1) {
                            var ele = json[i].id.split(',')[1]
                            $(ele).hide()
                        }
                    }
...
function getPkList(option) {
    ...
    if (option.url) { url = option.url } else { return }
    if (option.url) { id = option.id } else return
    if (option.url) { params = option.params }
    if (option.url) { async = option.async }
    if (option.url) { type = option.type }
    if (params === undefined) { params = {} }
    if (async === undefined) { async = true }
    if (type === undefined || type === '' || type == null) { type = 'POST' }
    if (option.normalSelect === undefined) { normalSelect = true }
    function isArrayS(o) {
