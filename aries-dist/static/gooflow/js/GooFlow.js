...
//初始化区域图的对象
	GooFlow.runOrEdit = property.toolBtns; //add by qiqingshan 是否有左侧工具栏
	...
	this.$flowid="newFlow_flow";//流程图的id
	this.$title="newFlow_flow";//流程图的名称
	this.$dtpFlag="N";//流程图的全事物开关
	this.$state="1";//流程图的启用、关闭开关
	this.$timeOut="-1";//流程图执行超时时间
	this.$nodeRemark={};//每一种结点或按钮的说明文字,JSON格式,key为类名,value为用户自定义文字说明
	this.$nowType="cursor";//当前要绘制的对象类型
	this.$insertOrUpdate = property.toolBtns;
	this.$lineData={};
	this.$lineCount=0;
	this.$nodeData={};
	this.$nodeCount=0;
	...
	if(property.haveHead){
      ...
      tmp+="<label title='"+(property.initLabelText||"newFlow_flow")+"' "
        +(GooFlow.prototype.color.main? "style='background:"+GooFlow.prototype.color.main+"'" : "")+">"+(property.initLabelText||"newFlow_flow")+"</label>";
		}
		...
		this.onBtnCloseClick=null;//新建流程图按钮被点中
		this.onBtnStartClick=null;//新建流程图按钮被点中
		this.onBtnEndClick=null;//新建流程图按钮被点中
		...
		this.$head.on("click",{inthis:this},function(e){
			...
			switch($(tar).attr("class")){
				...
				case "ico_close"	:if(This.onBtnCloseClick!=null)	This.onBtnCloseClick();break;
				case "ico_start"	:if(This.onBtnStartClick!=null)	This.onBtnStartClick();break;
                case "ico_end"	:if(This.onBtnEndClick!=null)	This.onBtnEndClick();break;
			}
		});
	}
	var toolWidth=0;
   ...
   // );
	...
	if(this.$editable){
	  ...
	  //为了结点而增加的一些集体delegate绑定  modify  by qiqingshan 主要是为了控制在批处理Job运行界面的一些属性
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		if(!this.$editable)	return;
		//modify qiqingshan 主要是控制在批处理Job运行界面连接线禁止点击事件
		$(this.$draw).delegate(tmpClk,"click",{inthis:this},function(e){
			if(GooFlow.runOrEdit && GooFlow.runOrEdit.length >0){
	...
	initGroup:function(width,height){
	  ...
	  this.$group.on("dblclick",{inthis:this},function(e){
		...
		var t=getElCoordinate(This.$workArea[0]);
		This.$textArea.val(oldTxt).css({display:"block",width:98,height:14,
			left:t.left+x-This.$workArea[0].parentNode.scrollLeft,
	...
	switchToolBtn:function(type){
		...
		if(this.$nowType=="group"){
			...
			for(var key in this.$areaDom)	{
				if (this.$areaDom.hasOwnProperty(key)) {
	...
	addNode:function(id,json){
		...
		if(json.type.indexOf(" round")<0){
			if(!json.width||json.width<98)json.width=98;
		...
		if(GooFlow.prototype.color.node){
			...
			}else if(json.type.indexOf("auth")>-1){
				this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.auth,"border-color":GooFlow.prototype.color.auth});
			}else if(json.type.indexOf("check")>-1){
				this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.check,"border-color":GooFlow.prototype.color.check});
			}else if(json.type.indexOf("confirm")>-1){
				this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.confirm,"border-color":GooFlow.prototype.color.confirm});
			}else if(json.type.indexOf("atom")>-1){
				this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.atom,"border-color":GooFlow.prototype.color.atom});
			}else if(json.type.indexOf("event")>-1){
				this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.event,"border-color":GooFlow.prototype.color.event});
			}else if(json.type.indexOf("eventService")>-1){
				this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.eventService,"border-color":GooFlow.prototype.color.eventService});
			}else if(json.type.indexOf("sub")>-1){
				this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.sub,"border-color":GooFlow.prototype.color.sub});
			}else if(json.type.indexOf("stdjob")>-1){
				this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.stdjob,"border-color":GooFlow.prototype.color.stdjob});
			}else if(json.type.indexOf("standby")>-1){
				this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.standby,"border-color":GooFlow.prototype.color.standby});
			}else if(json.type.indexOf("run")>-1){
				this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.run,"border-color":GooFlow.prototype.color.run});
			}else if(json.type.indexOf("fail")>-1){
				this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.fail,"border-color":GooFlow.prototype.color.fail});
			}else if(json.type.indexOf("finish")>-1){
				this.$nodeDom[id].css({"background-color":GooFlow.prototype.color.finish,"border-color":GooFlow.prototype.color.finish});
			}
			else{
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".rs_close","click",{inthis:this},function(e){
			if(!e)e=window.event;
			//modify qiqingshan 主要是为了控制在运行界面的job是不允许删除
			if(GooFlow.runOrEdit && GooFlow.runOrEdit.length > 0){
	...
	loadData:function(data){
		...
		if(data.state || data.state==1 || data.state==0)
			this.$state=data.state;
		if(data.dtpFlag) this.$dtpFlag=data.dtpFlag;
		...
		for(var j in data.lines) {
			if (data.lines.hasOwnProperty(j)) {
				this.addLine(j,data.lines[j]);
			}
		}
		for(var k in data.areas) {
			if (data.areas.hasOwnProperty(k)) {
	...
	exportData:function(){
		var ret={title:this.$title,flowid:this.$flowid,dtpFlag:this.$dtpFlag,timeOut:this.$timeOut,state:this.$state,nodes:this.$nodeData,lines:this.$lineData,initNum:this.$max};
...
///////////以下为有关画线的方法
	...
	addLine:function(id,json){
		...
		//设置$lineData[id]
		this.$lineData[id]=this.$lineData[id]==null?{}:this.$lineData[id];
		if(json.type){
	...
	resetLines:function(id,node){
		for(var i in this.$lineData){
			if (this.$lineData.hasOwnProperty(i)) {
